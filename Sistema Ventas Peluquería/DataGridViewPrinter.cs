 ...
 public class DataGridViewPrinter
 {
  ...
  private void Calculate(Graphics g)
  {
   if (PageNumber == 0) // Just calculate once
   {
    ...
    for (int i = 0; i < TheDataGridView.Columns.Count; i++)
    {
     ...
     tmpSize.Width = TheDataGridView.Columns[i].Width; //----------------------> con esto hago los encabezados de un ancho fijo
     ...
     // tmpSize.Width = TheDataGridView.Columns[i].Width; //-----------------> con esto hago las filas de un ancho fijo
     // if (tmpSize.Width > tmpWidth)
     //  tmpWidth = tmpSize.Width;
     //}

        //------------ añadido por mí --------------
     tmpFont = TheDataGridView.Rows[0].DefaultCellStyle.Font;
     if (tmpFont == null) // If the there is no special font style of the CurrentRow, then use the default one associated with the DataGridView control
         tmpFont = TheDataGridView.DefaultCellStyle.Font;

     //tmpSize = g.MeasureString("Anything", tmpFont);
     //RowsHeight.Add(tmpSize.Height);
    float altoMayorFila=0;
    int espacio = 10;
    for (int j = 0; j < TheDataGridView.Rows.Count-1; j++)
    {
        tmpSize = g.MeasureString(TheDataGridView.Rows[j].Cells[2].Value.ToString(), tmpFont);
        if (tmpSize.Height > altoMayorFila)
            altoMayorFila = tmpSize.Height;
        RowsHeight.Add(tmpSize.Height + espacio);
    }

     //RowsHeight.Add(altoMayorFila);
     tmpSize = g.MeasureString(TheDataGridView.Rows[0].Cells[i].EditedFormattedValue.ToString(), tmpFont);
     tmpSize.Width = TheDataGridView.Columns[i].Width; //-----------------> con esto hago las filas de un ancho fijo
        //-------------------------------------------
  ...
  private void DrawHeader(Graphics g)
  {
   ...
   // Calculating and drawing the HeaderBounds
      float anchoRectángulo=0; //sumamos el ancho de todas las columnas
      for (int i = 0; i < TheDataGridView.Columns.Count; i++)
          anchoRectángulo += TheDataGridView.Columns[i].Width;

   RectangleF HeaderBounds = new RectangleF(CurrentX, CurrentY, anchoRectángulo, RowHeaderHeight); //mColumnPointsWidth[mColumnPoint], RowHeaderHeight);
